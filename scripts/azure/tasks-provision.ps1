...
$tasksContainer = az cosmosdb sql container exists --account-name $account `
                                 --database-name $db `
                                 --name $tasksContainerName `
                                 --resource-group $rg
if ($tasksContainer -eq "false") {
  echo "Creating the empty $tasksContainerName container..."
